 ...
 * $Author: pbureau $
 * $Date: 2005/04/28 16:34:02 $
 * $Revision: 1.5 $
 * 
 * 
 * $Header: /home/cvs/libkorebot/src/tests/koala_demo_client.c,v 1.5 2005/04/28 16:34:02 pbureau Exp $
 */

/*--------------------------------------------------------------------*/
/*! 
 * \file   koala_demo_client.c koala demo program (client side)
 *
 * \brief
 * 	   Demo program to remote control the koala robot from 
 * 	   a wireless ethernet link. The client reads events from a joystick
 * 	   and send network command to the remote Koala.
 *
...
{
  static int speed=0,right=rightspeed,left=leftspeed;
  ...
  switch(actionflag)
  {
	...
	if(left<30)
	  left = 30;
	speed = -((value*maxspeed)/32767);
	sprintf(echoBuffer,"setspeed %d %d\n",(speed*right)/100,(speed*left)/100);
	sendflag = 1;
	timeflag = 1;
      }

      if(number == 0) {
	right = (((value+32767)*100)/65534);
	left  = 100 - right;
	if(abs(speed) < 5) {
...
void handle_joybutton(int number, int value, char * echoBuffer)
{
  switch(number)
  {
    case 0 :
      /* play sound */
      if(value)
	ksock_send_command(sock,"playsound %d",1);
      break;
    case 1 :
      /* play sound */
      if(value)
	ksock_send_command(sock,"playsound %d",4);
      break;
    case 2 :
      if(value)
	actionflag = ACT_BOTH;
    ...
    case 5 :
      /* play sound */
      if(value)
	ksock_send_command(sock,"playsound %d",2);
      break;
    case 6 :
      /* play sound */
      if(value)
	ksock_send_command(sock,"playsound %d",3);
      break;
    case 10 :
      /* init camera */
      if(value)
