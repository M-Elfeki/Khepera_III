...
static int encodeSideInfo( struct config_t *cfg, L3_side_info_t  *si );
static void encodeMainData(struct config_t *cfg, int l3_enc[2][2][samp_per_frame2],
                           L3_side_info_t  *si,
...
static void Huffmancodebits( struct config_t *cfg, BF_PartHolder **pph, int *ix, gr_info *gi );
...
void
L3_format_bitstream( struct config_t *cfg, int              l3_enc[2][2][samp_per_frame2],
                      ...
                      int              ancillary_bits )
{
    ...
    for ( gr = 0; gr < 2; gr++ )
        for ( ch =  0; ch < cfg->wave.channels; ch++ )
        {
    ...
    encodeSideInfo( cfg, l3_side );
    encodeMainData( cfg, l3_enc, l3_side, scalefac );
    ...
    */
    ...
    frameData->frameLength = cfg->mpeg.bits_per_frame;
    frameData->nGranules   = 2;
    frameData->nChannels   = cfg->wave.channels;
    ...
    for ( gr = 0; gr < 2; gr++ )
        ...
        for ( gr = 0; gr < 2; gr++ )
        {
            for ( ch = 0; ch < cfg->wave.channels; ch++ )
            {
                ...
                Huffmancodebits( cfg, &codedDataPH[gr][ch], ix, gi );
            } /* for ch */
        } /* for gr */
} /* main_data */
...
static int encodeSideInfo( struct config_t *cfg, L3_side_info_t  *si )
{
    ...
    headerPH = BF_addEntry( headerPH, cfg->mpeg.type,               1 );
/* HEADER HARDCODED SHOULDN`T BE THIS WAY ! */
    headerPH = BF_addEntry( headerPH, 1/*config.mpeg.layr*/,          2 );
    headerPH = BF_addEntry( headerPH, !cfg->mpeg.crc,               1 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.bitrate_index,      4 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.samplerate_index,   2 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.padding,            1 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.ext,                1 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.mode,               2 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.mode_ext,           2 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.copyright,          1 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.original,           1 );
    headerPH = BF_addEntry( headerPH, cfg->mpeg.emph,               2 );
...
/*
  ...
  */
static void
Huffmancodebits(struct config_t *cfg, BF_PartHolder **pph, int *ix, gr_info *gi )
{
    ...
    {
            {
                int *scalefac = &sfBandIndex[cfg->mpeg.samplerate_index+(cfg->mpeg.type*3)].l[0];
